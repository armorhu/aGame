package com.agame.services.load
{
	...
	public class LoadManager extends EventDispatcher
	{
		...
		{
			for each (var s:String in _eventList)
				...
				if (_status == STATUS_LOADING && _queue.length)
				{
					...
					for (var i:int=0, l:int=_queue.length; i < l && _loadingThreadCount < _maxThreadCount; i++, _loadingThreadCount++)
					{
						...
						if (!tempLoadItem)
							return;
						for (var o:* in _map)
							if (_map[o] == tempLoadItem)
								return;
						res=tempLoadItem.resource;
						request=new URLRequest(res.version_url); //+'?version='+Math.random());
						//		trace("-------------------------------------------------------------------",request.url);
...
//			dispatcher.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, httpstatusHandler,false,0,false);
			...
			if (dispatcher is Sound)
				dispatcher.addEventListener(Event.ID3, ID3Handler, false, 0, false);
		}
		...
		private function httpstatusHandler(e:HTTPStatusEvent):void
		{
			...
			else if (e.type == HTTPStatusEvent.HTTP_RESPONSE_STATUS)
			{ //回复的
				/**
...
//			dispatcher.removeEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS,httpstatusHandler);
		...
		private function progressHandler(e:ProgressEvent):void
		{
			...
			if (e.bytesTotal < 1000)
				...
				loaditem.percent=e.bytesLoaded / e.bytesTotal * 100;
		...
		private function errorHandler(e:*):void
		{
			...
			_itemFailed++;
			clearCurrentItem(e);
			reportFlow(tempItem, e.type == IOErrorEvent.IO_ERROR ? 2 : 4);
			afterComplete(tempItem);
		}
		 ...
		 * @param tempItem
		 */
		private function reportFlow(tempItem:LoadItem,from:int):void
		{
			if (flowReprotFunc == null)
				return;
			flowReprotFunc(tempItem.resource, Math.min(tempItem.bytesLoaded, tempItem.bytesTotal),from,tempItem.status);
		}
		...
		public function addAt(url:String, index:int, params:Object=null):Resource
		{
			...
			if (item == null)
			{
				item=new Resource();
				item.url=url;
				item.version_url=versionOf(url);
				...
				return null; //existing loading resource
			...
			if (_status == STATUS_LOADING && index == 0)
				return null;
		...
		public static function versionOf(url:String):String
		{
			if (version && version[url] != undefined)
			{
				var versionTag:String='_v_' + version[url];
				var versionIndex:int=url.lastIndexOf('.');
				if (versionIndex == -1)
					return url + versionTag;
				else
					return url.substr(0, versionIndex) + versionTag + url.substr(versionIndex);
			}
			else
				return url;
		}
		...
		private function removeItem(url:String, index:int=-1):Boolean
		{
			...
			{
				if (_status == STATUS_LOADING && _map[loader].url == url)
					return false;
			}
...
internal class LoadItem extends EventDispatcher
{
	...
	private function handleTimeComplete():void
	{
		...
		else if (resource.loader is LoaderInfo)
		{
			// 显示对象加载
